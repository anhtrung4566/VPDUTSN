    ...
    <section id="update-form-section" className="py-16 px-4 md:px-8 max-w-5xl mx-auto relative">
      ...
      <div className="luxury-card overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6 text-left">
          ...
          {/* STRUCTURED EXTRA FIELDS FROM FOOTER SUGGESTION */}
          <div className="p-5 rounded-2xl border border-gold-500/25 bg-gold-500/5 space-y-4">
            <h4 className="text-xs font-bold text-gold-700 dark:text-gold-400 flex items-center gap-1.5 uppercase tracking-wider">
              <Tag className="w-4 h-4 text-gold-500" /> Gợi ý cấu trúc đầu vào tối ưu (AI Suggester Fields)
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              
              {/* Keywords Tagging */}
              <div>
                <label className="text-xxs font-bold text-neutral-500 dark:text-neutral-400 uppercase block mb-1.5">Từ khóa chính</label>
                <div className="flex gap-1 mb-2">
                  <input
                    ...
                    className="flex-1 text-xs p-2 rounded-lg border border-neutral-300 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-neutral-800 dark:text-zinc-100 focus:outline-none"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        handleAddKeyword(e);
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-lg bg-neutral-200 dark:bg-zinc-800 hover:bg-neutral-300 dark:hover:bg-zinc-700 text-xs font-bold"
                  >
                    Thêm
                  </button>
                </div>
                <div className="flex flex-wrap gap-1">
                  {keywords.map((kw) => (
                    <span key={kw} className="text-[10px] font-bold inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gold-500/15 border border-gold-500/20 text-gold-700 dark:text-gold-300">
                      {kw}
                      <button type="button" onClick={() => handleRemoveKeyword(kw)} className="hover:text-rose-500">×</button>
                    </span>
                  ))}
                </div>
              </div>

              {/* Deadline & Opinion Flag */}
              <div className="space-y-4">
                <div>
                  <label className="text-xxs font-bold text-neutral-500 dark:text-neutral-400 uppercase block mb-1.5">Hạn cần xử lý nếu có</label>
                  <input
                    type="date"
                    value={deadline}
              ...
              {/* Coordination need */}
              <div>
                <label className="text-xxs font-bold text-neutral-500 dark:text-neutral-400 uppercase block mb-1.5">Cần phối hợp với đơn vị khác?</label>
                <div className="flex gap-2 mb-2">
                  <button
                    type="button"
                    onClick={() => setNeedsCooperation(true)}
                    className={`flex-1 text-xs py-1.5 rounded-lg font-bold ${needsCooperation ? 'bg-gold-500 text-zinc-950' : 'bg-neutral-100 dark:bg-zinc-800 text-neutral-500'}`}
                  >
                    Có phối hợp
                  </button>
                  <button
                    type="button"
                    onClick={() => setNeedsCooperation(false)}
                    className={`flex-1 text-xs py-1.5 rounded-lg font-bold ${!needsCooperation ? 'bg-gold-500 text-zinc-950' : 'bg-neutral-100 dark:bg-zinc-800 text-neutral-500'}`}
                  >
                    Không cần
                  </button>
